// spiprocessfuzzwin32.cpp : Defines the entry point for the application.
//

#include "stdafx.h"
#include "spiprocessfuzzwin32.h"
#include "FreeImage.h"
...
TCHAR szTitle[1024]={L"spiprocessfuzzwin32title"};					// The title bar text
TCHAR szWindowClass[1024]={L"spiprocessfuzzwin32class"};				// the main window class name
                     ...
                     int       nCmdShow)
{
	...
	if(nArgs>7)
	{
		global_duration_sec = atof(szArgList[7]);
	}
	...
	if(nArgs>17)
	{
		global_statictextcolor_red = atoi(szArgList[17]);
	}
	if(nArgs>18)
	{
		global_statictextcolor_green = atoi(szArgList[18]);
	}
	if(nArgs>19)
	{
		global_statictextcolor_blue = atoi(szArgList[19]);
	}
	if(nArgs>20)
	{
		wcscpy(szWindowClass, szArgListW[20]); 
	}
	if(nArgs>21)
	{
		wcscpy(szTitle, szArgListW[21]); 
	}
	if(nArgs>22)
	{
		global_begin = szArgList[22]; 
	}
	if(nArgs>23)
	{
		global_end = szArgList[23]; 
	}
